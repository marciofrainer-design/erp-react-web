...
export const reservaRegisterSchema = z.object({
  hospedeId: z.number({ error: "validation.hospedeId.required" }).int().positive("validation.hospedeId.positive"),
  quartoId: z.number({ error: "validation.quartoId.required" }).int().positive("validation.quartoId.positive"),
